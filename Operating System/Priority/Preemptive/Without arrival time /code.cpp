...
{
    string name;
    int at = 0, bt, ct, tat, wt, prio, rem;
};
...
int main()
{
    ...
    for (int i = 0; i < n; ++i)
    {
        cin >> process[i].name >> process[i].bt >> process[i].prio;
    ...
    while (completed < n)
    {
        ...
        for (int i = 0; i < n; ++i)
        {
            if (process[i].at <= current_time && !is_completed[i] && process[i].rem > 0)
            {
    ...
    {
        cout << p.name << '\t' << p.at << '\t' << p.bt << '\t' << p.prio << '\t' << p.ct << '\t' << p.tat << '\t'
             << p.wt << '\n';
    }
